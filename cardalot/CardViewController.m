...
- (IBAction)done:(id)sender {
    ...
    if ( [DeckController sharedInstance].decks.count >= 5 && [PurchasedDataController sharedInstance].goPro == NO && ![[DeckController sharedInstance].nameTags containsObject:self.deckTagCell.deckTagField.text]) {
        UIAlertController *deckLimitAlert = [UIAlertController alertControllerWithTitle:@"Go Pro for Unlimited Decks!" message:@"You have unlimited cards so if you rather not go pro, hold down on a deck to delete it."preferredStyle:UIAlertControllerStyleAlert];
